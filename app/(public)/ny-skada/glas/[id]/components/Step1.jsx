    ...
    return (
        <div className='space-y-6 bg-white border border-border rounded-xl p-6 shadow my-6'>
            ...
            <div>
                ...
                <input {...register('firstname')} type='text' name='firstname' className='w-full border border-border rounded px-3 py-2' />
                {errors.firstname && <p className='text-xs text-red-500 mt-1'>{errors.firstname.message}</p>}
            </div>
            <div>
                <label className='block text-xs font-bold mb-1 text-text-primary'>Efternamn</label>
                <input {...register('lastname')} type='text' name='lastname' className='w-full border border-border rounded px-3 py-2' />
                {errors.lastname && <p className='text-xs text-red-500 mt-1'>{errors.lastname.message}</p>}
            </div>

            <div>
                <label className='block text-xs font-bold mb-1 text-text-primary'>E-post</label>
                <input {...register('email')} type='email' name='email' className='w-full border border-border rounded px-3 py-2' />
                {errors.email && <p className='text-xs text-red-500 mt-1'>{errors.email.message}</p>}
            </div>
            <div>
                <label className='block text-xs font-bold mb-1 text-text-primary'>Telefon</label>
                <input {...register('phone')} type='tel' name='phone' className='w-full border border-border rounded px-3 py-2' />
                {errors.phone && <p className='text-xs text-red-500 mt-1'>{errors.phone.message}</p>}
            </div>
            <div>
                <label className='block text-xs font-bold mb-1 text-text-primary'>Person-/Organisationsnummer (YYMMDD-XXXX)</label>
                <input
